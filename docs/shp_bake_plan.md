...
- **지역 밖은 자동으로 잘린다**(2026-09-08 추가): 실데이터에서 200m 넘게 떨어진 셀은 Delaunay
  볼록껍질 안이라도 nodata가 된다. 껍질이 도(道) 경계의 오목한 부분을 거대한 삼각형으로 가로지르는
  탓에, 예전엔 각 지역 타일이 자기 영역 밖까지 값을 뻗쳐 이웃 지역 실데이터와 겹치며 mosaic을
  오염시켰다(**시도 경계쌍 58개 중 49개가 5m 초과 불일치, 최대 214m**). 내륙 타일에는 무해하다
  (걸리는 셀 0.0%). 2026-09-08 이전에 구운 타일은 `scripts/mask_far_cells.py`로 같은 상태로 만든다
...
- 중심선 props에 `차로수`·`도로폭`이 실려 런타임이 다차선 마킹을 생성한다.
  **연속수치지형도는 이 필드가 영문 코드**(`RVWD`=도로폭, `RDLN`=차로수, `RDDV`=도로구분)로 오는데
  별칭으로 잡아둬서 도엽별과 동일하게 동작한다(실측 의정부 중심선 23,802개 전부 폭·차로수 인식)
- **`--stream`은 도 영역 소스에 필수** — DEM과 같은 원리로 타일마다 그 영역만 읽는다.
  산출 타일은 전량 적재와 **바이트 동일**(실데이터 32타일·17,136폴리곤 대조 확인).
  작은 소스는 안 쓰는 게 빠르다(재읽기 비용 > 절약)
