...
- **≤500m — 단일 조립**: `/api/generate` 1회 → 전체 geometry를 한 번에 조립.
- **>500m — 타일 순차조립**: `/api/tile_plan`으로 고정 origin_offset + 타일 격자(기본 250m)를
  먼저 받고, 타일마다 `/api/generate_tile`을 **순차 호출해 하나씩 조립**(진행바 + 취소 버튼).
  매 단계 전송·메모리·빌드가 타일 하나 규모로 바운드돼 대반경(1~2km)에서 렉·크래시를 막는다.
  경계 건물은 "중심점이 그 타일인 것만" 조립해 중복 제거. **주의**: 타일링은 빌드 중 크래시를
  없앨 뿐, 2km 최종 모델(건물 수만·지형 삼각형 100만+)은 여전히 무겁다 → 지형 LOD는 별도 과제.
