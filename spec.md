...
**서빙(타일링)**: `road_manifest.json`만 git 추적, `roads_*.geojson`은 gitignore(DEM 타일과 동일 원칙).
런타임은 지역 파일을 json+shapely로 bbox 클립하는데, 메트로를 지역 1파일로 구우면 서울 311MB → 요청마다
전량 파싱(강남 250m 3분+/2GB). 그래서 DEM처럼 **2km 하드클립 타일**로 굽는다(`road_bake --tile-km 2` →
`bake_roads_tiled`; 타일 박스 교집합=공간 분할이라 중복·틈 없음, 갭채움 union도 타일 내부로 한정). 런타임
`store.find_road_files`가 질의와 겹치는 타일만 골라 `road.clip_*`(`_load_features`가 여러 타일 병합)로 읽어
**0.46s**로 떨어진다(서울 247타일, 결과 동일). 배포는 공개 GCS에 올리고 `ROAD_BASE=gs://<버킷>/roads`
(기본=로컬 `GEO_STORE`)로 서빙 — `config.road_file_path`가 gs→https 변환, `road._read_geojson_text`가
fetch+캐시. DEM은 GDAL `/vsicurl` 윈도우 읽기지만 도로는 HTTP fetch. **대전·서울 배포됨**. 미배포 지역은
조용히 생략(로컬 백엔드는 정상). 상세 `docs/deploy.md` §5.
...
3. **차선(다차선)** `clip_lane_markings` — 중심선 props의 실측 `차로수`·`도로폭`으로 차로수만큼 평행 차선
   ...
   리본)·`.3dm`(rhino, `lanes` 레이어 PolylineCurve)** 이 렌더 — 세 경로 정합. 클라우드 `.skp`(MCP)만 미포함(샌드박스).
...
`src/output/rhino.py` —
`write_3dm(solids, terrain, path, offset, cadastral=None, roads=None, sidewalks=None, water=None, ortho_image=None, ortho_extent_m=None, lanes=None, qa=None) -> str`
...
| `lanes` | 노랑 | `rhino3dm.PolylineCurve` (차선 마킹, F2·확장과 정합) |
| `qa_warn` / `qa_info` | 빨강 / 주황 | `rhino3dm.Mesh` 수직 핀 (자동 QA 결함, 심각도별) |
...
4. 지형 취득   manifest → find_tiles(겹치는 타일 전부) → clip_dem_mosaic(다중 타일 병합,
              DEM_TILE_BASE로 로컬/GCS) → build_tin(적응형 TIN 디스패처, §3.5) (layers.terrain 시)
...
src/
  ...
  terrain/
    store.py             load_manifest + find_tiles(겹치는 타일 전부·고해상도 우선) / find_tile(대표 1개)
                          + find_road_files(겹치는 도로 타일 전부) / find_road_file(대표 1개) / find_water_file(수계 GeoJSON)
    ...
    road_bake.py         A0010000 도로경계·A0020000 중심선(실측 도로폭·차로수)·A0033320 보도 SHP → 지역
                          ...
                          + bake_roads_tiled(--tile-km: 메트로용 2km 하드클립 타일링, STRtree+타일박스 교집합,
                          갭채움 union 타일 내부 한정 → 단일파일 311MB/요청당 3분+ 회피, 서울 247타일)
...
| 도로 R1~R3 (Phase R) | 노면(A0010000)·보도(A0033320) → F2/.3dm/.skp/확장 · 차선(A0020000 차로수 기반 **다차선**, 중앙선 실선/구분선 점선) → **F2·.3dm·확장**(라인/PolylineCurve/노란 면). 지형정합=버닝(절토/성토·스커트·IDW·클램프)+크라운 | ✅ |
| 출력 3경로 정합 | `.3dm`·F2·확장이 동일 레이어 렌더 — `.3dm`에 차선·QA 핀 추가(`rhino.py::_add_lanes`/`_add_qa_pins`), 확장에 지적 추가(`builder.rb::build_cadastral`) | ✅ |
...
| 메트로 도로 타일링 | `road_bake --tile-km 2`(`bake_roads_tiled`, 2km 하드클립·공간분할) + 런타임 `find_road_files` 다파일 클립(`_load_features`) — 단일 311MB/요청당 3분+ → 서울 247타일/0.46s(400배↑). 대전·서울 GCS 배포 | ✅ (서울, 타 광역시 반복) |
...
합성 데이터로만 동작 — 실제 키·파일 없이 전체 스위트 실행 가능. 현재 **325개 통과**
(`test_road`/`test_road_tiling`/`test_water`/`test_qa`/`test_trust_report`/`test_zoning`/`test_dem` 등 포함).
