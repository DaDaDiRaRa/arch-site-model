...
출력:  .skp (SketchUp MCP) + .3dm (rhino3dm)
...
| .3dm 출력 | rhino3dm 8.17.0 |
| 지형 DEM | rasterio + scipy (offline bake) |
...
MultiPolygon → 폴리곤별 분리 처리. 홀(중정) inner loop는 [목표].
...
`src/output/rhino.py` — `write_3dm(solids, terrain, path, offset, cadastral=None) -> str`

| 레이어 | 색상 | 객체 타입 |
|---|---|---|
| `buildings` | steel blue | `rhino3dm.Extrusion` (캡 포함) |
| `buildings_unverified` | orange | `rhino3dm.Extrusion` (policy=flag 시) |
| `terrain` | olive green | `rhino3dm.Mesh` |
...
8. .3dm 저장  write_3dm(solids, terrain, path, offset, cadastral)
9. provenance + origin_offset 기록
```

DEM 범위 밖 → `ok: true` + `warnings` 추가 + 건물만 생성 (조용한 fallback).
