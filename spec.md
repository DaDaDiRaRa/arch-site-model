...
```jsonc
{
  ...
  "qa": {                     // layers.qa=True 시 (§3.7)
    "findings": [{ "severity": "warning", "kind": "steep_seat", "message": "...", "at": [x, y, z], "name": "..." }],
    "summary": { "checked": 12, "warnings": 1, "info": 0 }
  },
  "warnings": []
}
```
...
각 finding은 `{severity(warning/info), kind, message, at:[x,y,z], name}`. 웹 UI가 결함 목록 패널을
...
src/
  ...
  qa.py                  run_qa() — 자동 QA/검증(§3.7): 건물 앉힘·겹침·footprint 유효성·지형 스파이크 → {findings, summary}
...
합성 데이터로만 동작 — 실제 키·파일 없이 전체 스위트 실행 가능. 현재 **291개 통과**
(`test_road`/`test_road_bake`/`test_water`/`test_qa`/`test_tiles_stream` 등 신규 포함).
