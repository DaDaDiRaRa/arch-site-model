...
- [ ] **전국 5m DEM 확장**: 6개 광역단체(대전·서울·부산·대구·울산·세종 120타일) GCS 라이브. 건물·지적은
      ...
      **대상지형지물 = 지형·교통·수계·시설 4개만** 체크(건물·식생·경계·주기는 VWorld/타 앱과 중복).
      SHP 보관 = `D:\APPS\SHP\ctnu_도영역\<시도>\`(zip 그대로, 규칙은 그 폴더 `_받는_방법.txt`).
      연속본은 **EPSG:5179**·영문 필드(`CONT`/`NUME`)·**MultiLineString 포함** — 셋 다 대응 완료.
      대용량 한 파일이라 `bake_tiled(stream=True)`(CLI `--stream`)로 타일마다 그 영역만 읽어 굽는다
      (경기도 정점 1억 개 = 전량 적재 시 12GB → 스트리밍은 타일당 4초·메모리 고정, 산출 DEM 동일).
      지역 추가 = 도 영역 확보(사람 손) → `bake_tiled --stream`→`dem_to_cog`→`gcloud storage cp`→
      `manifest.json` 커밋(반복). **인천·광주 보류 해제**(2026-08-27): 인천 데이터 확보, 광주는
      2026-07-01 **전남광주통합특별시** 출범(전남+광주 통합, 약칭 광주특별시)으로 개편 확정.
      VWorld 지오코딩은 신·구 표기 모두 같은 좌표를 반환하는 것으로 실측 확인(전남광주통합특별시/
      광주광역시/전라남도/광주특별시 4형태 전부 OK) → **주소 입력 경로는 무영향**.
      **2026-08-25 결정: 30m 전역 폴백(terrarium 등) 도입 안 함** — 미커버 지역도 SHP 베이크로만 채움
      ...
      단 **국토정보플랫폼 공개DEM 90m**(전국·무료·`.img`·EPSG:5179, 전국 한 파일본은 이미 5186)은 미커버
      지역 폴백 후보로 남아 있음(경쟁사 김동욱 도구가 쓰는 그 데이터, 실측 5m 대비 중앙값 3.1m·p90 20.9m).
      **실행 절차서 `docs/shp_bake_plan.md`**. 상세 [[nationwide-dem-ngii-source]]·[[ngii-continuous-map-route]].
- [ ] **전남 표고점·도로·수계 보완(포털 데이터 결함)**: 2026-08-28 기준 `D:\APPS\SHP\ctnu_도영역\`에
      **17개 시도 확보**(강원 3.9G 포함, 분할압축 26조각 무결 확인). **전라남도만 부분 확보** —
      포털이 `전라남도`/`광주광역시` 어느 이름으로 내려받아도 **내용이 광주 데이터**이고(2회 재현 → 전송
      사고 아니라 포털 데이터 결함), 등고선 분할압축만 마지막 볼륨이 광주 파일로 바뀌어 있었다.
      → 고아 조각(.z01~.z12, 1.26GB)에서 **local file header 직접 스캔으로 등고선 복구 성공**
      (`_01/_02` 완전 + `_03` 끝 308KB 유실 후 .shx 재생성, 합계 **248,797피처·262×170km·2.0GB**,
      정점밀도가 경기도와 동급이라 사실상 전역). 순천 3km 5m 베이크 실측 통과(유효 100%, 5~345m).
